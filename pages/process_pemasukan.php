...
if ($action == 'tambah') {
    ...
    $sql = "INSERT INTO pemasukan (kategori_id, tanggal, jumlah, deskripsi" . 
           (columnExists($conn, 'pemasukan', 'user_id') ? ", user_id" : "") . 
           ") VALUES (?, ?, ?, ?" . (columnExists($conn, 'pemasukan', 'user_id') ? ", ?" : "") . ")";
    $stmt = $conn->prepare($sql);
    if (columnExists($conn, 'pemasukan', 'user_id')) {
    ...
    if ($stmt->execute()) {
        $_SESSION['message'] = "Pemasukan berhasil ditambahkan!";
...
if ($action == 'ubah') {
    ...
    if (columnExists($conn, 'pemasukan', 'user_id')) {
        $sql_check = "SELECT id FROM pemasukan WHERE id = ? AND user_id = ?";
    ...
    $sql = "UPDATE pemasukan SET kategori_id=?, tanggal=?, jumlah=?, deskripsi=? WHERE id=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("isdsi", $kategori_id, $tanggal, $jumlah, $deskripsi, $id);
    
    if ($stmt->execute()) {
        $_SESSION['message'] = "Pemasukan berhasil diupdate!";
        $_SESSION['alert_type'] = "success";
    } else {
        $_SESSION['message'] = "Error: " . $stmt->error;
        $_SESSION['alert_type'] = "danger";
...
if ($action == 'hapus') {
    ...
    if ($stmt->execute()) {
        $_SESSION['message'] = "Pemasukan berhasil dihapus!";
        $_SESSION['alert_type'] = "success";
    } else {
        $_SESSION['message'] = "Error: " . $stmt->error;
        $_SESSION['alert_type'] = "danger";
